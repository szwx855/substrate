...
where
	T::AccountId: UncheckedFrom<T::Hash> + AsRef<[u8]>
{
	gas_meter.charge(CodeToken::UpdateRefcount(code_size::<T>(&code_hash)?))?;
	<CodeStorage<T>>::mutate(code_hash, |existing| {
...
where
	T::AccountId: UncheckedFrom<T::Hash> + AsRef<[u8]>
{
	if let Ok(len) = code_size::<T>(&code_hash) {
	...
	T::AccountId: UncheckedFrom<T::Hash> + AsRef<[u8]>
{
	...
	// based upon the code size.
	if let Some((_, gas_meter)) = &mut reinstrument {
		gas_meter.charge(CodeToken::Load(code_size::<T>(&code_hash)?))?;
	}
...
fn code_size<T: Config>(code_hash: &CodeHash<T>) -> Result<u32, DispatchError>
where
	T::AccountId: UncheckedFrom<T::Hash> + AsRef<[u8]>
{
